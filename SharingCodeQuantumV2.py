...
def plotClusters(listCities, listClusters, nameOfpng,timer):
    plt.figure()
    ...
    for i in range(0, len(listClusters)):
        ...
        for j in range(0, len(listClusters[i])):
            ...
            #If there is not too much cities, we plot the city number
            if (len(listCities) < 26):
...
def plotTSP(listCities, listPositionsPerCluster, nameOfpng, timer, timerTotal):
    plt.figure()
